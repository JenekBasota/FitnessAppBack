    ...
    try:
        if user and auth_Blueprint.hasher.verify(user.password, password):
            access_token = create_access_token(
                identity=user.id, expires_delta=datetime.timedelta(days=31)
            )
            return jsonify(
                {
                    "msg": "Login success",
...
def register_step_first():
    ...
    user = auth_Blueprint.user_table.CheckUniqueEmailOrLogin(
...
def register_step_two():
    ...
    if (
        type(username) != str
        or type(email) != str
        or (type(gender) != str and gender is not None)
        or type(password) != str
        or (type(height) != int and height is not None)
        or (type(weight) != int and weight is not None)
        or type(programs) != int
    ):
        return jsonify({"msg": "Incorrect data type detected", "status": 400})
    ...
    access_token = create_access_token(
        identity=user_id, expires_delta=datetime.timedelta(days=31)
    )
    return jsonify(
        {
            "msg": "User created",
